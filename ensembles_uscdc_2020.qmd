...
auscafe |>
  filter(year(Month) >= 2014, year(Month) <= 2018) |>
  autoplot(Turnover) +
...
fc2 <- bind_rows(ensemble, combination) |>
  ...
  as_fable(index=Month, key=.model, distribution=Turnover, response="Turnover")
fc2 |> autoplot(train |> filter(year(Month) >= 2017))
```
