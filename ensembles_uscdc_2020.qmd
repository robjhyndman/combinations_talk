...
## The tsibble class {auto-animate="true"}
...
## Using the fable package {auto-animate="true"}
...
```{r ensemble_samples0, echo=FALSE}
fit <- auscafe |>
  filter(year(Month) <= 2017) |>
   model(
    arima = ARIMA(Turnover ~ pdq(d=1) + PDQ(D=1))
...
auscafe |>
  ...
  ylim(2800, 4850) +
  autolayer(auscafe |> filter(year(Month) >= 2014))
```
...
## Forecasting ensemble (linear pooling) {.smaller}
...
den_plot <- function(prob, actual=3) {
  p <- tibble(
      y = seq(0, 10, l = 101),
      f = dgamma(y, 2)
    ) |>
  ...
  if(!is.null(prob)) {
    ...
    p <- p + geom_line(aes(x = y, y = f), col = "blue", linetype = 2,
      data = tibble(y = rep(qactual, 2), f = c(dgamma(qactual, 2), -0.1))
    )  +
    geom_label(aes(x = qactual, y = -0.01, label = paste(sprintf("%.3f", prob), "quantile")), col = "blue")
  }
  p +
...
crps_plot <- function(thisprob, actual=3) {
  ...
  p <- p3data |>
    ...
    geom_polygon(
      fill = "#ffbbbb",
      data = bind_rows(p3data, tibble(p = thisprob, Q = 0))
    ) +
  ...
  if(thisprob > 0.995)
    p <- p + geom_text(aes(x=0.35,y=5,label="Area = CRPS"), color="red", size=10)
  p
}
```

```{r pdf000, dependson='qs'}
den_plot(NULL, actual=NULL)
```
...
den_plot(0.5) / score_plot(0.5)
```


## Evaluating probabilistic forecasts

```{r pdfqs1, dependson='qs'}
den_plot(0.9) / score_plot(0.9)
```
...
(plot_spacer() | den_plot(0.5)) / (crps_plot(0.5) | score_plot(0.5))
```
...
p1 +
  geom_line(aes(y = Turnover), data = auscafe |> filter(year(Month) >= 2017))
```
