...
den_plot <- function(prob, actual=3) {
  ...
  if(!is.null(prob)) {
    ...
    geom_label(aes(x = qactual, y = 0.4, label = paste(sprintf("%.3f", prob), "quantile")), col = "blue")
  }
  p +
...
crps_panel <- function(prob) {
  (plot_spacer() | den_plot(prob)) / (crps_plot(prob) | score_plot(prob))
}
...
for(i in seq_along(prob)) {
  if(i < 10)
    filename <- paste0("figs/crps0",i,".jpg")
  else
    filename <- paste0("figs/crps",i,".jpg")
  ggsave(file = filename, plot = crps_panel(prob[i]),
         width=25, height=15, units="cm", dpi=300)
}
# make a GIF with ImageMagick
system("convert -delay 10 figs/crps*.jpg crps.gif")
